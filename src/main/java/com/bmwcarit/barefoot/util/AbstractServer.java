...
import com.bmwcarit.barefoot.util.AbstractServer.RESULT;
import com.bmwcarit.barefoot.util.AbstractServer.StopException;

/**
 * Abstract server implementation for hosting a service that can be accessed via
 * TCP/IP socket.
 */
public class AbstractServer
{
	...
	/**
	 * Creates a {@link AbstractServer} object. It expects the following
	 * properties:
	 * <ul>
	 ...
	 *            {@link Properties} object containing all necessary server
	 *            settings.
	 * @param responseFactory
	 *            {@link ResponseFactory} object that generates a
	 *            {@link ResponseHandler} for each request message which, in
	 *            turn, processes requests and creates response messages.
	 */
	public AbstractServer(Properties serverProperties, ResponseFactory responseFactory)
	{
		...
		this.maxRequestTime = Integer.parseInt(serverProperties.getProperty("server.timeout.request", "15000"));
		this.maxResponseTime = Integer.parseInt(serverProperties.getProperty("server.timeout.response", "60000"));
		this.maxConnectionCount = Integer.parseInt(serverProperties.getProperty("server.connections", "20"));
		this.responseFactory = responseFactory;
		
		this.clientHandler = new ClientHandler(null);
	...
	/**
	 * Abstract factory for generation of a {@link ResponseHandler} object for
	...
	/**
	 * Abstract response handler that processes request messages and generates
	 * response messages.
	 */
	protected static abstract class ResponseHandler implements Callable<Tuple<RESULT, String>>
	{
		...
		/**
		 * Processes a request message, generates a response, and returns a
		 * {@link RESULT}.
		 *
		 * @param request
		 *            Request message.
		 * @param response
		 *            {@link StringBuilder} for generation of the response.
		...
		public Tuple<RESULT, String> getTuple() throws IOException
		{
			RESULT result = response(request, response);
			return new Tuple<>(result, response.toString());
		}
	...
	public class ClientHandler extends Thread
	{
		...
		public String handleRequest(String request)
		{
			...
			try
			{
				Tuple<RESULT, String> response = responseFactory.response(request).getTuple();

				//logger.trace("{}:{} response code {}", client.getInetAddress().getHostAddress(), client.getPort(), response.one());
				
				sb.append(response.one());

				if (response.one() == RESULT.SUCCESS)
				{
					sb.append(response.two());
				}
			}
			catch (Exception e)
			{
				e.printStackTrace();
				//logger.error("{}:{} client handler stopped due to unexpected error - {}", client.getInetAddress().getHostAddress(), client.getPort(), e.getMessage());
			}
		...
		public void run()
		{
			try
			{
				...
				try
				{
					...
					logger.trace("{}:{} request - {}", client.getInetAddress().getHostAddress(), client.getPort(), request);
				}
				catch (TimeoutException e)
				{
					requestHandler.cancel(true);

					logger.error("{}:{} request handler timeout", client.getInetAddress().getHostAddress(), client.getPort());
				...
				{
					logger.error("{}:{} request handler exception - {}", client.getInetAddress().getHostAddress(), client.getPort(), e.getMessage());

					throw new StopException();
				}

				FutureTask<Tuple<RESULT, String>> responseHandler = new FutureTask<>(responseFactory.response(request));
				executor.execute(responseHandler);

				try
				{
					Tuple<RESULT, String> response = responseHandler.get(maxResponseTime, TimeUnit.MILLISECONDS);
				...
				catch (TimeoutException e)
				{
					...
					logger.error("{}:{} response handler timeout", client.getInetAddress().getHostAddress(), client.getPort());
				...
				{
					logger.error("{}:{} response handler exception - {}", client.getInetAddress().getHostAddress(), client.getPort(), e.getMessage());
			...
			{
				logger.trace("{}:{} client handler stopped execution", client.getInetAddress().getHostAddress(), client.getPort());
			}
			catch (Exception e)
			{
				logger.error("{}:{} client handler stopped due to unexpected error - {}", client.getInetAddress().getHostAddress(), client.getPort(), e.getMessage());
			...
			finally
			{
				...
				{
					logger.error("{}:{} connection lost", client.getInetAddress().getHostAddress(), client.getPort());
				}
				logger.trace("{}:{} connection closed ({} open)", client.getInetAddress().getHostAddress(), client.getPort(), openConnectionCount.decrementAndGet());
			}
		}
	}
	...
	public void runServer() throws RuntimeException
	{
		...
		try
		{
			...
			{
				/*
				 * Uses this work-around to ensure a maximum number of open
				 * connections to clients. This is necessary because the
				 * ServerSocket's backlog parameter has no effect on the accept
				 * method, i.e., it still accepts more connections than
				 * specified with backlog. Note that incoming connections are
				 * still queued and, hence, any incoming connection that exceeds
				 * the maximum number is not refused on the client side.
				 */
				...
				logger.trace("{}:{} connection accepted ({} open)", client.getInetAddress().getHostAddress(), client.getPort(), openConnectionCount.incrementAndGet());
